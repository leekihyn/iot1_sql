...
var treeview;// tree view 작성을 위한 변수 

function onBound(){
	treeview = $('#treeview').data('kendoTreeView'); // kendo가 가지고 있음 kendoGrid 는 grid를 옮김 
}
...
function callbackForTreeItem(result){ //databaseList를 불러오는 콜백함수 트리구조로 박힘 treeview.append({})
	if(result.error){
		alert(result.error);
		return;
	}
...
function toolbarEvent(e){ // 아이디가 btnConnect의 text()를 불러옴 diNum을 보내서 해당 db에 접근함  
